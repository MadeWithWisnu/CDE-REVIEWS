...
import { CAM_DEBTOR_TYPES } from '../data/camTypes.js';
...
const isValid = () => appNo.value.trim() && debtorType.value;
...
function submit() {
  ...
  router.push({
    path: `/cam/${encodeURIComponent(appNo.value.trim())}`,
    query: { debtorType: debtorType.value },
  });
}
</script>
  ...
  <div class="search-page">
    <div class="search-bar">
      <div class="field">
        ...
        <input
          v-model="appNo" type="text" placeholder="e.g. 23182604001"
      ...
      <div class="field">
        ...
        <select v-model="debtorType" class="select" :class="{ invalid: touched && !debtorType }">
          ...
          <option v-for="d in CAM_DEBTOR_TYPES" :key="d.key" :value="d.key">{{ d.label }}</option>
        </select>
      </div>
    ...
    <p v-if="touched && !isValid()" class="form-error">Please complete Application No and Debtor Type.</p>

    <div class="hint-panel">
      <div class="hint-title">How to use</div>
      <ol class="hint-list">
        <li>Enter the <b>Application No</b> of the application you want to review.</li>
        <li>Select the <b>Debtor Type</b> — Personal or Company.</li>
        <li>Click <b>Search</b> to open the Credit Approval Memorandum (CAM) report.</li>
      </ol>
    </div>
  </div>
</template>
